...
| --- | ---: | ---: | --- |
| Framework | 3.11.2 (Phase 5 complete) | 3.x; 4.0.0 after the deck-building/BO3 layer | Per release stage |
...
Version 3.10.0 establishes the boundary: `DeckSpec` stores full Main, Extra, and Side sections
with construction identity independent of filename and ordering, while `DeckProfile` contains
Main/Extra only and is the sole stable image visible to the current single-duel policy. `.ydk`
Side sections now participate in resource parsing and separate compatibility checks but never
enter `DuelState` model inputs. `DuelState` separately retains the stable initial image and the
dynamic remaining cards. `MatchContext` and `DuelSummary` live outside single-duel state as
future BO3/deck-building extension points and currently produce no policy inputs.

Version 3.10.1 implements the private training-trajectory registration path. Workers deduplicate
each stable per-duel image by `profile_id` and store an `int32 deck_profile_index` per sample.
Dynamic remaining-card `deck_idx/deck_mask` fields stay per-step; three exact-token static fields
move into a compact metadata catalog that the Trainer merges, remaps, and reconstructs element
for element before each PPO mini-batch. Existing network, ONNX, and policy numerics therefore stay
...
`deck_gate` starts at zero, modulation is bounded, and a light per-duel persisted deck-condition mask limits early over-reliance while keeping PPO old/new log-probs under the same condition. Deck summaries also enter policy/value heads directly.

Version 3.11.0 implements this structure with low-rank layered generators: global/deck branches
are independent, every layer's attention and FFN receive separate γ/β, and tanh bounds total
modulation to ±0.5. The per-channel deck gate starts at zero while its generator is nonzero, so
the first main-loss step can move the gate. A separate Worker RNG writes a false
`deck_film_mask` for 10% of complete training duels, and central inference plus PPO trajectories
reuse that value; deployment defaults to true. The direct deck policy/value path is never masked.
...
- [x] **Phase 5: layered FiLM/history**—separate modulation and 16 compact events.
