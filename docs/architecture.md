...
新增卡及其效果不会破坏旧前缀；修改旧卡结构字段或代码向量会被明确拒绝。当前 3.9.0
仍由 Encoder 输出原有语义张量，3.9.1 才切换为网络内查表并删除轨迹重复数据。
...
3.9.0 将运行时结构语义统一编译为 `[card_token, effect_slot]` 静态表。Encoder 的兼容
接口和网络代码向量读取同一进程缓存，避免重复解析 25 MB 知识库和重复装载向量矩阵；
三件套不完整、槽位/向量索引错位或旧卡语义身份不一致时均在建网前停止。
