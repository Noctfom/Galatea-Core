...
OCGCore 在某些效果（如"禁止令"、"抹杀之指名者"）下会发送 `MSG_SELECT_CARD` (type=142)，要求玩家从数万张卡片中选择一张。如果直接把数万个合法选项丢给神经网络，动作空间的维度会瞬间爆炸，导致显存溢出和训练无法收敛。
...
YGOPro 的某些交互消息（如 `MSG_SELECT_UNSELECT` type=26）允许玩家**多次选择 + 取消选择**多张卡片，直到满足条件后才提交。传统 RL 框架难以处理这种"多步组合动作"，因为每步可选动作集合会动态变化。
...
| 消息类型 | 场景 | 宏动作处理 |
|----------|------|------------|
| `MSG_SELECT_UNSELECT` (26) | 同调召唤选素材 | 一步选择所有素材 |
| `MSG_SELECT_CARD` (15) | 融合召唤选素材 | 一步选择所有素材 |
| `MSG_SELECT_PLACE` (18) | 多卡放置 | 一步决定所有放置位置 |
