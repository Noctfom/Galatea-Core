...
3.9.0 在此映射之上建立按精确卡片 token × Lua 效果槽对齐的模型侧静态表。身份哈希计算的是各槽实际结构字段和逻辑代码向量，因此增量生成导致 `.npy` 物理行号变化不会误判；只追加新卡不破坏旧前缀，但修改旧卡语义会在 PTH/ONNX/部署包加载前被拒绝。本批仍通过兼容接口输出旧张量，避免资产身份与轨迹去重同时改动。
