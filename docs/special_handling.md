...
在 **🧠 语义知识库引擎** 模块中：
1. 勾选 **🌐 仅同步远程语义资产** 时只下载完整四文件基座，不解析本地 Lua
2. 勾选 **🧬 本地提取/接续语义** 时才扫描本地脚本，并自动增量维护结构语义与代码向量
3. 两项可同时使用以“先同步、再接续”；物理清空且不启用同步时，本地更新从零构建
4. 点击 **开始提取卡片语义** 并等待所选操作完成

Hash 聚类标签由规范化代码的 MD5 决定，本身是确定性的。v3.6.0 修复的是字段去重使用无序 `set` 后再截断造成的非确定性。3.7.0 起远程同步与本地提取显式分离；远程没有 Hash 映射时，框架仍会从知识库已有的 `CUSTOM_HASH_*` 标签重建接续记录。

### V3 观测与效果槽审计

3.6.1 引入 V3 观测审计；3.6.3 起，训练和竞技场默认关闭，需显式开启 `--protocol-audit`，RuleBot 自检仍默认采集。运行时只累计 Core 消息与连锁公开字段，按进程写入 `system_logs/protocol_v3_audit/`；它不读取 Lua，也不参与动作选择或奖励计算。
...
3.9.0 在此映射之上建立按精确卡片 token × Lua 效果槽对齐的模型侧静态表。身份哈希计算的是各槽实际结构字段和逻辑代码向量，因此增量生成导致 `.npy` 物理行号变化不会误判；只追加新卡不破坏旧前缀，但修改旧卡语义会在 PTH/ONNX/部署包加载前被拒绝。3.9.1 删除逐步展开语义：场面与卡组复用已有 token，连锁和最近历史传递紧凑效果槽 ID；槽位未知或静态表没有对应槽时仍按原规则回退整卡语义，不丢失原有观测。
3.9.2 将该表写成可校验的 NPZ/JSON 运行资产；源语义或权威词表通过正式同步/生成流程变化时会先删除旧编译表，再一次性重建。spawn Worker 仅读取 JSON 运行目录，避免为效果槽绑定重新常驻整张数值表。
...
3.8.1 起，Type 20 与 Type 23 会先把每张候选的解放值/双值参数送入 Pass1，再生成完整组合；Type 23 没有 Core 取消位，候选池不再伪造 `-1`。所有 Type 15/20/23 响应在进入模型池前都会按内核同等边界复核，取消选项即使经过 120 项加权缩减也固定保留。

3.8.2 起，场上、墓地和除外区的同名卡不再按卡密折叠，因为它们可能拥有不同的正规出场位、原因或公开关系；隐藏/重置区只有在提示参数和已知状态完全一致时才使用等价副本。此修复不改变 Core 候选索引，仍由既有 5,000 项上限与加权缩减控制池大小。
...
Type 26 的 `response_value` 只用于学习语义，物理响应始终是 `count=1 + index`；Finish/Cancel 才使用 `int32(-1)`。终止动作放在候选池首位，避免超大候选列表把它截到 120 个网络槽之外。所有字节响应最终进入 512 字节保活缓冲，与 Core 的 `SIZE_RETURN_VALUE` 完全一致。

训练保留整局内由完整状态键生成的紧凑访问计数，因此“进入选择 → Cancel/Unselect → 回到原状态”的 A→B→A 往返不会因中间状态变化而漏检。同一完整状态下前 4 次合法撤回没有额外惩罚；第 5 次 Cancel/Unselect 起施加 `-0.005` 步奖励，一般动作仍从第 10 次重复起处理。动作始终留在合法池中并按 PPO 策略随机采样，不使用训练期硬屏蔽。
...
| `MSG_SORT_CHAIN/SORT_CARD` (21/25) | 同时触发效果/卡片排序 | 包装合法次序 |
