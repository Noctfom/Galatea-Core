...
class PPOTrainer:
    def __init__(self, save_dir="./models", deck_dir="./decks", net_config=None, resume_path=None,
        ...
        input_specs = {
            ...
            'card_overlay_rest_idx': ((120, CARD_ADDITIONAL_OVERLAY_SLOTS), torch.int16),
            'card_reason_bytes': ((120, CARD_REASON_BYTES), torch.uint8),
            'card_status_bits': ((120, CARD_STATUS_BITS), torch.uint8),
            'card_owner_role': ((120,), torch.uint8),
            'card_relation_idx': ((120, CARD_RELATION_SLOTS), torch.uint8),
            'card_relation_type': ((120, CARD_RELATION_SLOTS), torch.uint8),
            'card_counter_type_bytes': ((120, CARD_COUNTER_SLOTS, CARD_COUNTER_TYPE_BYTES), torch.uint8),
            'card_counter_value': ((120, CARD_COUNTER_SLOTS), torch.float16),
            'field_zone_mask': ((FIELD_ZONE_BYTES,), torch.uint8),
            ...
            'event_mask': ((TRANSITION_EVENT_HISTORY_SIZE,), torch.bool),
            'event_card_idx': ((TRANSITION_EVENT_HISTORY_SIZE,), torch.int32),
            'event_effect_slot': ((TRANSITION_EVENT_HISTORY_SIZE,), torch.uint8),
            'event_actor_role': ((TRANSITION_EVENT_HISTORY_SIZE,), torch.uint8),
            'event_prompt': ((TRANSITION_EVENT_HISTORY_SIZE,), torch.uint8),
            'event_operation': ((TRANSITION_EVENT_HISTORY_SIZE,), torch.uint8),
            'event_summon_method': ((TRANSITION_EVENT_HISTORY_SIZE,), torch.uint8),
            'event_phase': ((TRANSITION_EVENT_HISTORY_SIZE,), torch.uint8),
            'event_boundary': ((TRANSITION_EVENT_HISTORY_SIZE,), torch.uint8),
            'event_result_bytes': ((TRANSITION_EVENT_HISTORY_SIZE, TRANSITION_EVENT_RESULT_BYTES), torch.uint8),
            'event_message_bytes': ((TRANSITION_EVENT_HISTORY_SIZE, TRANSITION_EVENT_MESSAGE_BYTES), torch.uint8),
            'event_source_context': ((TRANSITION_EVENT_HISTORY_SIZE, TRANSITION_EVENT_LOCATION_DIM), torch.uint8),
            'event_target_card_idx': ((TRANSITION_EVENT_HISTORY_SIZE, TRANSITION_EVENT_TARGET_SLOTS), torch.int32),
            'event_target_context': ((TRANSITION_EVENT_HISTORY_SIZE, TRANSITION_EVENT_TARGET_SLOTS, TRANSITION_EVENT_LOCATION_DIM), torch.uint8),
            'event_target_mask': ((TRANSITION_EVENT_HISTORY_SIZE, TRANSITION_EVENT_TARGET_SLOTS), torch.bool),
            'event_lp_delta': ((TRANSITION_EVENT_HISTORY_SIZE, TRANSITION_EVENT_LP_DIM), torch.float16),
            'event_zone_delta': ((TRANSITION_EVENT_HISTORY_SIZE, 2 * TRANSITION_ZONE_COUNT), torch.int8),
            'event_chain': ((TRANSITION_EVENT_HISTORY_SIZE, TRANSITION_EVENT_CHAIN_DIM), torch.uint8),
            'event_counts': ((TRANSITION_EVENT_HISTORY_SIZE, TRANSITION_EVENT_COUNT_DIM), torch.uint8),
